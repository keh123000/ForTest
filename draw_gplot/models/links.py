...
def link_get_by_userid_and_source_and_target(user_id, source, target):
    link = Links.objects.filter(user_id=user_id, source_node_id=source, target_node_id=target).first()
    return link


def links_get_by_user_id(user_id):
    links = Links.objects.filter(user_id=user_id,status=1).order_by('update_time').all()
    return links
